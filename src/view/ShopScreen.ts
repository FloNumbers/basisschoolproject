class ShopScreen {
    ...
    public draw() {
        this.canvas.writeTextToCanvas('Winkel', 150, this.canvas.getCenter().X, this.canvas.getCenter().Y -260, "black")
        this.canvas.writeTextToCanvas('Hints :', 100, this.canvas.getCenter().X - 393, this.canvas.getCenter().Y -100, "black")
        this.drawShopBoxes(200, 175)
        this.canvas.writeTextToCanvas('Background :', 100, this.canvas.getCenter().X- 545, this.canvas.getCenter().Y +100, "black")
        this.drawShopBoxes(200, -25)
        this.canvas.writeTextToCanvas('Music :', 100, this.canvas.getCenter().X -410, this.canvas.getCenter().Y +300, "black")
    ...
    public drawShopBoxes(xBox: number, yBox: number) {
        for (let index = 0; index < 3; index++) {
            ...
            this.canvas.writeTextToCanvas("Koop", 50, this.canvas.getCenter().X -xBox+50 + (index * 130), this.canvas.getCenter().Y -yBox-20, "black")
